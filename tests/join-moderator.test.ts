...
describe("moderateJoinRequest", () => {
  ...
  it("declines a group join request when the applicant is not subscribed to the required channel", async () => {
    ...
    ).resolves.toEqual({ action: "decline", channelStatus: "left" });

    expect(api.getChatMember).toHaveBeenCalledWith("@my_channel", 42);
    expect(api.declineChatJoinRequest).toHaveBeenCalledWith(-100100, 42);
    expect(api.approveChatJoinRequest).not.toHaveBeenCalled();
  });

  it("declines a group join request when Telegram says the applicant was not found in the channel", async () => {
    ...
    ).resolves.toEqual({ action: "decline", channelStatus: "left", lookupError: "user_not_found" });
  ...
  it("does not approve or decline when the channel membership lookup fails for a reason other than missing user", async () => {
